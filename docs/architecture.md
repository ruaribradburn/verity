...
This diagram reflects the system described in [`PDR.md`](../PDR.md): entry points → ingestion → orchestrator → specialized agents → shared structured context → synthesis → user-facing output, with persistence and model dependencies.
...
flowchart TB
  ...
  subgraph Agents["Specialized agents"]
    ...
    SYN[Synthesis / output agent<br/>single user-facing narrative · sections · citations]
  end
  ...
  subgraph Output["User-visible output"]
    UI[Structured UI:<br/>summary · bias · credibility · entities · gaps · recommendations]
